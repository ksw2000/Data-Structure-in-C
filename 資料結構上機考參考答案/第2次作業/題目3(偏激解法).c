...
//Phase1: Read file and make table
    ...
    int num_big_flush=0, num_flush=0, num_4_tong=0, num_3_tong=0, num_pair=0, num_single=0;
    int mark[13];
...
//How to combine to get the answer we want.
    ...
    for(i=0; i<4; i++){
        for(j=0; j<13; j++){
            if(flush_table[i][j]==1){
                if(flush_table[i][(j-1)%13]==0 && flush_table[i][(j+1)%13]==0){
                    //這個順一定要選
                    if(j==9){
                        num_big_flush++;
                    }else{
    ...
    for(i=0; i<4; i++){
        for(j=0; j<13; j++){
            if(flush_table[i][j]==1){
                ...
                k=(k+1)%13;
                //k值為連號最起始之j值
                for(counter=1; flush_table[i][(k+counter)%13]!=0; counter++);
    ...
    printf("\nanswer:\n%d\n%d\n%d\n%d\n%d\n",\
    num_big_flush, num_flush, num_ti_gi, num_hulu, num_pair);
