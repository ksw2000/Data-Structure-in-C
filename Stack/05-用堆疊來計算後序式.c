/*
    ...
    另外，也可以參考 Fundamentals of data structures in c 2nd edition 
    p.131 Figure 3.14 或我也有放在本資料夾的 README.md 內
*/
...
int eval(char* expr) {
    ...
    // n: 讀取字串時的 index ，因為要從 0 開始讀所以預設為 0
    // symbol: 字串第 n 個字的字元，利用指標變數回傳(C的函數不能有兩個回傳值)
    int n = 0;
    char symbol;
    ...
    while (token != eos) {
        if (token == operand) {  // token 是數字時
            ...
            // 意思就是如果我得到 symbol='1' 那 symbol - '0' 可視為 '1' - '0' 即
            // 49 - 28 = 1
