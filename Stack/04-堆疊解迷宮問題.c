/*
 ...
 -> 0 0 0 0 0 0
    ...
    STEP3: 一直走直到走到終點，若是一直退回上一步，退到沒上一步可退(即pop不出東西)，那就印出沒有路徑
*/
...
int main(){
    ...
    for(; !flag;){
        for(i=0; i<8; i++){
            ...
            //printf("d(%d,%d)\n",now_row + direct[i].delta_row, now_col + direct[i].delta_col);
            if(inBorder(now_row + direct[i].delta_row, now_col + direct[i].delta_col)){
                if(maze[now_row + direct[i].delta_row][now_col + direct[i].delta_col] == 0){
                    //確定要走這條，將 now_row, now_col 改成新值 (但位子是記錄舊的)
...
*/
bool inBorder(int row, int col){
    if(row<0 || row>=MAZE_ROW) return false;
    if(col<0 || col>=MAZE_COL) return false;
    return true;
}
